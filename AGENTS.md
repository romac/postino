...
- `Codec.derived` also works for sealed trait hierarchies in the declaration-order case when every child subtype has its own `Codec`: `Mirror.SumOf` children are assigned `u32` discriminants `0..n-1`. Scala 3 `enum` cases are not auto-derived today. Use `Postino.sum[A].variant(discriminant, codec).build` for `#[repr]`, serde tags, sparse discriminants, or any schema where Scala declaration order does not exactly match Rust.
